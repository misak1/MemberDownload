...
/**
 * Storage driver for use against PDO
 *
 ...
 * @package    Auth
 * @author     Original Author Martin Jansen <mj@php.net>
 * @author     Original Author Adam Ashley <aashley@php.net>
 * @copyright  2001-2006 The PHP Group
 * @license    http://www.php.net/license/3_01.txt  PHP License 3.01
 * @version    2011-09-05 copied from DB.php
...
class Auth_Container_PDO extends Auth_Container
{
     ...
     */
    function Auth_Container_PDO($dsn)
    {
        $this->_setDefaults();
echo "<pre>";
...
echo "</pre>";
    ...
    {
        $this->log('Auth_Container_PDO::_connect() called.', AUTH_LOG_DEBUG);
 
    try {
        if (is_string($dsn) || is_array($dsn)) {
            //$this->db = new PDO($dsn, $this->options['db_options']);
            $this->db = new PDO($dsn, $this->options['db_user'],$this->options['db_password']);
        } else {
    ...
    function _prepare()
    {
        ...
        if ($this->options['auto_quote'] && $this->db->dsn['phptype'] != 'sqlite') {
            if (strpos('.', $this->options['table']) === false) {
                // Postgresql quote is " but quote function add ' .
                $this->options['final_table'] = $this->db->quote($this->options['table']);
            } else {
                $t = explode('.', $this->options['table']);
                for ($i = 0, $count = count($t); $i < $count; $i++)
                    $t[$i] = $this->db->quote($t[$i]);
                $this->options['final_table'] = implode('.', $t);
            }
            $this->options['final_usernamecol'] = $this->db->quote($this->options['usernamecol']);
            $this->options['final_passwordcol'] = $this->db->quote($this->options['passwordcol']);
        } else {
...
*/
        ...
        if (isset($this->options['db_fields'])) {
            if (is_array($this->options['db_fields'])) {
                ...
                if (strlen($this->options['db_fields']) > 0) {
                    if ($this->options['auto_quote']) {
                        return $this->db->quote($this->options['db_fields']);
                    } else {
    ...
    {
        $this->log('Auth_Container_PDO::fetchData() called.', AUTH_LOG_DEBUG);
        ...
        $query = "SELECT ".$sql_from.
                ...
                " WHERE ".$this->options['final_usernamecol']." = :username";
    ...
    function listUsers()
    {
        $this->log('Auth_Container_PDO::listUsers() called.', AUTH_LOG_DEBUG);
        $err = $this->_prepare();
        if ($err !== true) {
            return PEAR::raiseError($err->getMessage(), $err->getCode());
        }
    ...
    {
        $this->log('Auth_Container_PDO::addUser() called.', AUTH_LOG_DEBUG);
        $err = $this->_prepare();
        if ($err !== true) {
            return PEAR::raiseError($err->getMessage(), $err->getCode());
        }
        ...
        if (is_array($additional)) {
            foreach ($additional as $key => $value) {
                if ($this->options['auto_quote']) {
                    $additional_key .= ', ' . $this->db->quote($key);
                } else {
                    $additional_key .= ', ' . $key;
                }
    ...
    {
        $this->log('Auth_Container_PDO::removeUser() called.', AUTH_LOG_DEBUG);
    ...
    {
        $this->log('Auth_Container_PDO::changePassword() called.', AUTH_LOG_DEBUG);
        $err = $this->_prepare();
        if ($err !== true) {
            return PEAR::raiseError($err->getMessage(), $err->getCode());
        }
